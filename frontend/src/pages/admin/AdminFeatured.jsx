        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
          ...
          {editingItem && editingItem.itemType !== 'custom' && (
            <div className="bg-blue-50 text-blue-800 p-4 rounded-lg mb-6 text-sm">
              <span className="font-bold">Note:</span> You are overriding details for the linked item: <strong className="text-blue-900 text-lg">{editingItem.referenceId ? editingItem.referenceId.name || editingItem.referenceId.dealNumber : 'Unknown Item'}</strong>. The original deal/menu item will remain unchanged elsewhere.
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" placeholder={editingItem && editingItem.itemType !== 'custom' && editingItem.referenceId ? (editingItem.referenceId.name || editingItem.referenceId.dealNumber) : ''} value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" required={!editingItem || editingItem.itemType === 'custom'} />
            </div>
